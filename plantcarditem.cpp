...
PlantCardItem::PlantCardItem(const Plant *plant, bool smaller) : checked(true), percent(0), overlayImage(new QGraphicsPixmapItem)
{
    ...
    QPixmap image = ImageCache->load(plant->cardGif);//设置image为其对应的卡片
    checkedImage = image.copy(0, 0, image.width(), image.height() / 2);//取下面的
    uncheckedImage = image.copy(0, image.height() / 2, image.width(), image.height() / 2);//取上面的
    if (!smaller) {
    ...
    : tooltipText(new QGraphicsTextItem)
{
    ...
    setRect(tooltipText->boundingRect());//按照文本绘制
