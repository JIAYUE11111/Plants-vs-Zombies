...
{
    QSoundEffect *firepea=new QSoundEffect;
    firepea->setSource(QUrl::fromLocalFile((":/audio/firepea.wav")));
    firepea->setLoopCount(1);  //循环次数
    firepea->play();
...
void SunFlowerInstance::initTrigger()
{
        ...
        double fromX = coordinate.getX(col) - sunGif->boundingRect().width() / 2 + 15,
                toX = coordinate.getX(col) -  QRandomGenerator::global()->generate() % 80,
        ...
        QEasingCurve::OutCurve).finish()
                .move(QPointF(toX, toY)).scale(1.0).speed(0.2).shape( QEasingCurve::InCurve).finish(
                    onFinished);
        (new Timer(picture, ( QRandomGenerator::global()->generate() % 8000 + 1000), [this] { initTrigger(); }))->start();
}
...
{
    QSoundEffect *lawnmower=new QSoundEffect;
    lawnmower->setSource(QUrl::fromLocalFile((":/audio/lawnmower.wav")));
    lawnmower->setLoopCount(1);  //循环次数
    lawnmower->play();
