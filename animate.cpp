...
Animate::Animate(QGraphicsItem *item, QGraphicsScene *scene)
    ...
    m_shape(QEasingCurve::InOutSine),
...
Animate &Animate::shape(QEasingCurve  shape)
{
    m_shape = shape;
    return *this;
}
...
Animate &Animate::finish(std::function<void(bool)> functor)
{
    ...
    qDebug()<<"type:"<<type<<" animation"<<animation;
    if (!(type & (MOVE | SCALE | FADE))) {
        qDebug()<<"type & (MOVE | SCALE | FADE";
        if (animation) {
            animation->anim->stop();
    ...
    if (keyFrame.duration <= 0) {
        ...
        QObject::connect(animation->anim, &QTimeLine::finished, [item, animation] {
            ...
            if (!animation->frames.isEmpty()) {
                ...
                //delete animation->anim;
                delete animation;
                setAnimation(item, nullptr);//这个显示房间的东西不会再有动画
            }
        });
