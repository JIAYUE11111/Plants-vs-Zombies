...
void Zombie::update()
{
    QPixmap pic = ImageCache->load(staticGif);
    width = pic.width();
    height = pic.height();
}
...
FlagZombie::FlagZombie()
{
    ...
    staticGif = path + "0.gif";
    normalGif = path + "FlagZombie.gif";QSoundEffect *splat1=new QSoundEffect;splat1->setSource(QUrl::fromLocalFile((":/audio/splat1.wav")));splat1->setLoopCount(1);splat1->play();//QSound::play(":/audio/splat1.wav");
...
void ZombieInstance::judgeAttack()
{
    ...
    if (col >= 1 && col <= 9) {
        ...
        std::sort(keys.begin(), keys.end(), [](int a, int b) { return b < a; });
        for (auto key: keys) {
            plant = plants[key];
...
void ZombieInstance::normalAttack(PlantInstance *plantInstance)
{
    ...
    {
        QSoundEffect *chomp=new QSoundEffect;
        chomp->setSource(QUrl::fromLocalFile((":/audio/chomp.wav")));
        chomp->setLoopCount(1);
        chomp->play();//QSound::play(":/audio/chomp.wav");
    }
    else
    {
        QSoundEffect *chompsoft=new QSoundEffect;
        chompsoft->setSource(QUrl::fromLocalFile((":/audio/chompsoft.wav")));
        chompsoft->setLoopCount(1);
        chompsoft->play();
    }
...
{
    QSoundEffect *splat=new QSoundEffect;

    splat->setLoopCount(1);
    //QSound::play(":/audio/chomp.wav");
    switch (QRandomGenerator::global()->generate() % 3) {
    case 0:
        splat->setSource(QUrl::fromLocalFile((":/audio/splat1.wav")));
        splat->play();
        //QSound::play(":/audio/splat1.wav");
        break;
    case 1:
        //QSound::play(":/audio/splat2.wav");
        splat->setSource(QUrl::fromLocalFile((":/audio/splat2.wav")));
        splat->play();
        break;
    default:
        //QSound::play(":/audio/splat3.wav");
        splat->setSource(QUrl::fromLocalFile((":/audio/splat3.wav")));
        splat->play();
        break;
    }
}
...
{
    QSoundEffect *effect = new QSoundEffect; // 设置父对象以确保正确的内存管理
    effect->setSource(QUrl("qrc:/audio/frozen.wav")); // 使用 qrc 路径对资源文件进行访问
    effect->setLoopCount(1);
    effect->play();
    //effect->setVolume(0.25f); // 根据需要设置音量
...
void ZombieInstance::playFireballAudio()
{
    if (QRandomGenerator::global()->generate() % 2)
    {
        ...
        QSoundEffect *effect = new QSoundEffect; // 设置父对象以确保正确的内存管理
        effect->setSource(QUrl("qrc:/audio/ignite.wav")); // 使用 qrc 路径对资源文件进行访问
        effect->setLoopCount(1);
        effect->play();}
    else
    {    QSoundEffect *effect = new QSoundEffect; // 设置父对象以确保正确的内存管理
        effect->setSource(QUrl("qrc:/audio/ignite2.wav")); // 使用 qrc 路径对资源文件进行访问
        effect->setLoopCount(1);
        effect->play();}
...
void ConeheadZombieInstance::playNormalballAudio()
{
    ...
    {
        QSoundEffect *effect = new QSoundEffect; // 设置父对象以确保正确的内存管理
        effect->setSource(QUrl("qrc:/audio/plastichit.wav")); // 使用 qrc 路径对资源文件进行访问
        effect->setLoopCount(1);
        effect->play();}
    else
...
void BucketheadZombieInstance::playNormalballAudio()
{
    if (hasOrnaments) {
        if (QRandomGenerator::global()->generate() % 2)
        {
            ...
            QSoundEffect *effect = new QSoundEffect; // 设置父对象以确保正确的内存管理
            effect->setSource(QUrl("qrc:/audio/shieldhit.wav")); // 使用 qrc 路径对资源文件进行访问
            effect->setLoopCount(1);
            effect->play();
        }
        else{
            ...
            QSoundEffect *effect = new QSoundEffect; // 设置父对象以确保正确的内存管理
            effect->setSource(QUrl("qrc:/audio/shieldhit2.wav")); // 使用 qrc 路径对资源文件进行访问
            effect->setLoopCount(1);
            effect->play();
        }
    }
...
void PoleVaultingZombieInstance::normalAttack(PlantInstance *plantInstance)
{
    if (lostPole)
        ...
        QSoundEffect *effect = new QSoundEffect; // 设置父对象以确保正确的内存管理
        effect->setSource(QUrl("qrc:/audio/grassstep.wav")); // 使用 qrc 路径对资源文件进行访问
        effect->setLoopCount(1);
        effect->play();
        ...
        isAttacking = true;
        altitude = 2;
        (new Timer(picture, 500, [] {
            QSoundEffect *effect = new QSoundEffect; // 设置父对象以确保正确的内存管理
            effect->setSource(QUrl("qrc:/audio/polevault.wav")); // 使用 qrc 路径对资源文件进行访问
            effect->setLoopCount(1);
            effect->play();
            //QSound::play(":/audio/polevault.wav");
        }))->start();
        QUuid plantUuid = plantInstance->uuid;
