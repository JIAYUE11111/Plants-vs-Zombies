...
{
    return x2c.second[std::lower_bound(x2c.first.begin(),x2c.first.end(),x) - x2c.first.begin()];
}

int Coordinate::getRow(double y) const
{
    return y2r.second[std::lower_bound(y2r.first.begin(),y2r.first.end(), y) - y2r.first.begin()];
}

double Coordinate::getX(int c) const
{
    return c2x.second[std::lower_bound(c2x.first.begin(),c2x.first.end(), c) - c2x.first.begin()];
}

double Coordinate::getY(int r) const
{
    return r2y.second[std::lower_bound(r2y.first.begin(),r2y.first.end(), r) - r2y.first.begin()];
}
