...
GameScene::GameScene(GameLevelData *gameLevelData)
    ...
    background(new QGraphicsPixmapItem(ImageCache->load(gameLevelData->backgroundImage))),
    gameGroup(new QGraphicsItemGroup),
    infoText(new QGraphicsSimpleTextItem),
    infoTextGroup(new QGraphicsRectItem(0, 0, 900, 50)),
    menuGroup(new MouseEventsPixmapItem(ImageCache->load("interface/Button.png"))),
    sunNumText(new QGraphicsSimpleTextItem(QString::number(gameLevelData->sunNum))),
    sunNumGroup(new QGraphicsPixmapItem(ImageCache->load("interface/SunBack.png"))),
    selectCardButtonReset(new MouseEventsPixmapItem(ImageCache->load("interface/SelectCardButton.png"))),
    selectCardButtonOkay(new MouseEventsPixmapItem(ImageCache->load("interface/SelectCardButton.png"))),
    selectCardTextReset(new QGraphicsSimpleTextItem(tr("Reset"))),
    selectCardTextOkay(new QGraphicsSimpleTextItem(tr("Go"))),
    selectingPanel(new QGraphicsPixmapItem(ImageCache->load("interface/SeedChooser_Background.png"))),
    cardPanel(new QGraphicsItemGroup),
    shovel(new QGraphicsPixmapItem(ImageCache->load("interface/Shovel.png"))),
    shovelBackground(new QGraphicsPixmapItem(ImageCache->load("interface/ShovelBack.png"))),
    ...
    waveTimer(nullptr), monitorTimer(new QTimer(this)), waveNum(0)
{
    ...
    if (gameLevelData->showScroll) {
        ...
        for (const auto &zombieData: gameLevelData->zombiesData) {
            Zombie *item = getZombieProtoType(zombieData.eName);
            if(item->canDisplay) {
                for (int i = 0; i < zombieData.num; ++i) {
                    yPos.push_back(qFloor(100 +   QRandomGenerator::global()->generate() % 400));
        ...
        std::mt19937 rng(rd()); // 使用随机设备初始化Mersenne-Twister生成器
        std::shuffle(zombies.begin(), zombies.end(),rng);
        for (int i = 0; i < zombies.size(); ++i) {
            MoviePixmapItem *pixmap = new MoviePixmapItem(zombies[i]->standGif);
            QSizeF size = pixmap->boundingRect().size();
            pixmap->setPos(qFloor(1115 +  QRandomGenerator::global()->generate() % 200) - size.width() * 0.5, yPos[i] - size.width() * 0.5);
    ...
    addItem(menuGroup);
    connect(menuGroup, &MouseEventsPixmapItem::mouseclicked, [this] {
        monitorTimer->stop();
        delete monitorTimer;
    ...
    if (gameLevelData->canSelectCard && gameLevelData->maxSelectedCards > 0) {
        ...
        for (auto item: plantProtoTypes.values()) {
            ...
            connect(plantCardItem, &PlantCardItem::clicked, [this, item, plantCardItem] {
                ...
                tap->setSource(QUrl::fromLocalFile((":/audio/tap.wav")));
                tap->setLoopCount(1);  //循环次数
                tap->play();
                ...
                *deselectConnnection = connect(selectedPlantCardItem, &PlantCardItem::clicked, [this, selectedPlantCardItem, deselectFunctor] {
                    ...
                    for (int i = std::find(selectedCards.begin(),selectedCards.end(), selectedPlantCardItem) - selectedCards.begin() + 1; i != selectedCards.size(); ++i)
        ...
        connect(selectCardButtonOkay, &MouseEventPixmapItem::clicked, [this] {
            QSoundEffect *tap=new QSoundEffect;
            tap->setSource(QUrl::fromLocalFile((":/audio/tap.wav")));
            tap->setLoopCount(1);  //循环次数
            tap->play();
        ...
        connect(selectCardButtonReset, &MouseEventPixmapItem::clicked, [this] {
            QSoundEffect *tap=new QSoundEffect;
            tap->setSource(QUrl::fromLocalFile((":/audio/tap.wav")));
            tap->setLoopCount(1);  //循环次数
            tap->play();
...
void GameScene::loadAcessFinished()
{
    ...
    if (gameLevelData->showScroll) {
        backgroundMusic->setSource(QUrl("qrc:/audio/Look_up_at_the_Sky.mp3"));
...
void GameScene::letsGo()
{
    ...
    Animate(sunNumGroup, this).move(QPointF(105, 0)).speed(0.5).finish();
    if (gameLevelData->hasShovel)
        Animate(shovelBackground, this).move(QPointF(235, 0)).speed(0.5).finish();
    if (!gameLevelData->showScroll || !gameLevelData->canSelectCard) {
        cardPanel->setPos(-100, 0);
        Animate(cardPanel, this).move(QPointF(0, 0)).speed(0.5).finish();
    }
    ...
    connect(this, &GameScene::mousePress, [this](QGraphicsSceneMouseEvent *event) {
        ...
        if (i != selectedPlantArray.size()) {
            ...
            QSoundEffect *seedlift=new QSoundEffect;
            seedlift->setSource(QUrl::fromLocalFile((":/audio/seedlift.wav")));
            seedlift->setLoopCount(1);  //循环次数
            seedlift->play();
            //QSound::play(":/audio/seedlift.wav");
            choose = 1;
        }
        //是否捡起了铲子
            ...
            shovel->setSource(QUrl::fromLocalFile((":/audio/shovel.wav")));
            shovel->setLoopCount(1);  //循环次数
            shovel->play();
            //QSound::play(":/audio/shovel.wav");
            choose = 2;
        }
        else
            return;
            ...
            if (choose == 1) {
                ...
                if (e->button() == Qt::LeftButton && item->canGrow(xPair.second, yPair.second)) {
                    ...
                    *connection = connect(growGif, &MoviePixmapItem::finished, [growGif, connection]{
                    ...
                    updateSunNum();
                    // TODO: Plant Water
                    QSoundEffect *plant=new QSoundEffect;
                    if (QRandomGenerator::global()->generate() % 2)
                    {
                        plant->setSource(QUrl::fromLocalFile((":/audio/plant1.wav")));
                        plant->setLoopCount(1);  //循环次数
                    }
                    else
                    {
                        plant->setSource(QUrl::fromLocalFile((":/audio/plant2.wav")));
                        plant->setLoopCount(1);  //循环次数
                        plant->play();}
                }
                else {//种不成
                ...
                if (e->button() == Qt::LeftButton && (plant = getPlant(e->scenePos()))) {
                    plantDie(plant);
                    QSoundEffect *plant2=new QSoundEffect;
                    plant2->setSource(QUrl::fromLocalFile((":/audio/plant2.wav")));
                    plant2->setLoopCount(1);  //循环次数
                    plant2->play();
                }
                else
                {
...
QPair<MoviePixmapItem *, std::function<void(bool)> > GameScene::newSun(int sunNum)
{
    ...
    *connection = connect(sunGif, &MoviePixmapItem::click, [this, sunGif, sunNum, timer] {
        ...
        if (*timer)
            delete *timer;
        QSoundEffect *points=new QSoundEffect;
        points->setSource(QUrl::fromLocalFile((":/audio/points.wav")));
        points->setLoopCount(1);  //循环次数
        points->play();
...
void GameScene::beginSun(int sunNum)
{
    ...
    double toX = coordinate.getX(1 +  QRandomGenerator::global()->generate() % coordinate.colCount()),
        toY = coordinate.getY(1 +  QRandomGenerator::global()->generate() % coordinate.rowCount());
    sunGif->setPos(toX, -100);
    sunGif->start();
    Animate(sunGif, this).move(QPointF(toX, toY - 53)).speed(0.04).finish(onFinished);

    (new Timer(this, ( QRandomGenerator::global()->generate() % 9000 + 100), [this, sunNum] { beginSun(sunNum); }))->start();
}
...
{
    QSoundEffect *awooga=new QSoundEffect;
    awooga->setSource(QUrl::fromLocalFile((":/audio/awooga.wav")));
    awooga->setLoopCount(1);  //循环次数
    awooga->play();
    ...
    *playGroan = [this, playGroan] {
        QSoundEffect *groan=new QSoundEffect;
        switch ( QRandomGenerator::global()->generate() % 6) {
        case 0:
            groan->setSource(QUrl::fromLocalFile((":/audio/groan1.wav")));
            groan->setLoopCount(1);  //循环次数
            groan->play();
            break;
        case 1:
            groan->setSource(QUrl::fromLocalFile((":/audio/groan2.wav")));
            groan->setLoopCount(1);  //循环次数
            groan->play();
            break;
        case 2:
            groan->setSource(QUrl::fromLocalFile((":/audio/groan3.wav")));
            groan->setLoopCount(1);  //循环次数
            groan->play();
            break;
        case 3:
            groan->setSource(QUrl::fromLocalFile((":/audio/groan4.wav")));
            groan->setLoopCount(1);  //循环次数
            groan->play();
            break; break;
        case 4:
            groan->setSource(QUrl::fromLocalFile((":/audio/groan5.wav")));
            groan->setLoopCount(1);  //循环次数
            groan->play();
            break;
        default:
            groan->setSource(QUrl::fromLocalFile((":/audio/groan6.wav")));
            groan->setLoopCount(1);  //循环次数
            groan->play();
            break;
        }
...
void GameScene::prepareGrowPlants(std::function<void(void)> functor)
{
    ...
    backgroundMusic->stop();
    backgroundMusic->blockSignals(false);
    backgroundMusic->setSource(QUrl("qrc:/audio/readysetplant.mp3"));
    backgroundMusic->play();
...
void GameScene::advanceFlag()
{
    ...
    selectFlagZombie(flagToSumNum.second[std::lower_bound(flagToSumNum.first.begin(),flagToSumNum.first.end(), waveNum) - flagToSumNum.first.begin()]);
}
...
void GameScene::selectFlagZombie(int levelSum)
{
    ...
    if (gameLevelData->largeWaveFlag.contains(waveNum)) {
        QSoundEffect *siren=new QSoundEffect;
        siren->setSource(QUrl::fromLocalFile((":/audio/siren.wav")));
        siren->setLoopCount(1);  //循环次数
        siren->play();
        //QSound::play(":/audio/siren.wav");
    ...
    std::sort(zombiesCandidate.begin(), zombiesCandidate.end(), [](Zombie *a, Zombie *b) { return a->level < b->level; });
    while (levelSum > 0) {
        while (zombiesCandidate.last()->level > levelSum)
            zombiesCandidate.pop_back();
        Zombie *item = zombiesCandidate[QRandomGenerator::global()->generate() % zombiesCandidate.size()];
        levelSum -= item->level;
        zombies.push_back(item);
    }
    for (int i = 0; i < zombies.size(); ++i) {
        ...
        (new Timer(this, i * timeout, [this, zombie] {
            int row;
            do {
                row = QRandomGenerator::global()->generate() % coordinate.rowCount() + 1;
            ...
            std::sort(zombieRow[row].begin(), zombieRow[row].end(), [](ZombieInstance *a, ZombieInstance *b) {
...
{
    return std::lower_bound(craters.begin(),craters.end(), qMakePair(col, row)) != craters.end();
}

bool GameScene::isTombstone(int col, int row) const
{
    return std::lower_bound(tombstones.begin(),tombstones.end(), qMakePair(col, row)) != tombstones.end();
}
...
void GameScene::addTrigger(int row, Trigger *trigger)
{
    ...
    std::sort(plantTriggers[row].begin(), plantTriggers[row].end(), [](const Trigger *a, const Trigger *b) {
        return a->to < b->to;
    });
}
...
void GameScene::beginBGM()
{
    ...
    backgroundMusic->stop();
    backgroundMusic->blockSignals(false);
    backgroundMusic->setSource(QUrl(gameLevelData->backgroundMusic));
    backgroundMusic->play();
}
...
void GameScene::gameWin()
{
    ...
    backgroundMusic->stop();
    backgroundMusic->blockSignals(false);
    backgroundMusic->setSource(QUrl("qrc:/audio/winmusic.mp3"));
    backgroundMusic->play();
