          ...
          backgroundMusic(new QMediaPlayer(this))
{
    ...
    connect(adventureButton, &HoverChangedPixmapItem::hoverEntered, [] {
        QSoundEffect *effect=new QSoundEffect;
        effect->setSource(QUrl::fromLocalFile((":/audio/bleep.wav")));
        effect->setLoopCount(1);  //循环次数
        effect->play(); });
    connect(survivalButton, &HoverChangedPixmapItem::hoverEntered, [] {
        QSoundEffect *effect=new QSoundEffect;
        effect->setSource(QUrl::fromLocalFile((":/audio/bleep.wav")));
        effect->setLoopCount(1);  //循环次数
        effect->play();});
    connect(challengeButton, &HoverChangedPixmapItem::hoverEntered, [] {
        QSoundEffect *effect=new QSoundEffect;
        effect->setSource(QUrl::fromLocalFile((":/audio/bleep.wav")));
        effect->setLoopCount(1);  //循环次数
        effect->play();});
